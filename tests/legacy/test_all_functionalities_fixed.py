#!/usr/bin/env python3
"""
Test complet de toutes les fonctionnalités avec les corrections
"""
...
def test_all_functionalities_fixed():
    """Test complet de toutes les fonctionnalités avec corrections"""
    print("🧪 TEST COMPLET - TOUTES LES FONCTIONNALITÉS (CORRIGÉ)")
    print("=" * 70)
    ...
    # Test checkout
    try:
        response = requests.post(f"{BASE_URL}/orders/checkout", headers=user_headers)
        if response.status_code == 200:
            order = response.json()
            test_order_id = order["order_id"]  # Correction: utiliser order_id
    ...
    try:
        response = requests.post(f"{BASE_URL}/admin/products", json={
            "name": "Test Produit Admin Final",
            "description": "Produit de test final créé par admin",
            "price_cents": 1999,
            "stock_qty": 5,
            "active": True
    ...
    try:
        response = requests.post(f"{BASE_URL}/support/threads", json={
            "subject": "Test support client final",
    ...
    if test_thread_id:
        try:
            response = requests.post(f"{BASE_URL}/support/threads/{test_thread_id}/messages", json={
                "content": "Message de test support client final"
            }, headers=user_headers)
            if response.status_code == 200:
    ...
    if test_thread_id:
        try:
            response = requests.post(f"{BASE_URL}/admin/support/threads/{test_thread_id}/messages", json={
                "content": "Réponse admin de test final"
            }, headers=admin_headers)
            if response.status_code == 200:
    ...
    if all_tests_passed:
        ...
        print(f"   Le support admin et toutes les autres fonctionnalités sont opérationnelles !")
        return True
    else:
...
if __name__ == "__main__":
    success = test_all_functionalities_fixed()
